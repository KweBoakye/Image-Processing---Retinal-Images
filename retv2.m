retina2 = imread('retina2.png'); %read in image
gr1 = rgb2gray(retina2); 
mono2= imread('monochrome2.png');
mono2= rgb2gray(mono2);
mono2 = imbinarize(mono2);
mono2 = imresize(mono2,[267 280]);
...
fret1 = imfilter(retina2,g1,'replicate');% the image is filtered in each direction
fret2 = imfilter(retina2,g2,'replicate');
fret3 = imfilter(retina2,g3,'replicate');
fret4 = imfilter(retina2,g4,'replicate');
fret5 = imfilter(retina2,g5,'replicate');
fret6 = imfilter(retina2,g6,'replicate');
fret7 = imfilter(retina2,g7,'replicate');
fret8 = imfilter(retina2,g8,'replicate');

fret9 =fret1 +fret2 +fret3 +fret4 +fret5 +fret6 +fret7 +fret8 ;% the sum

lab_he = rgb2lab(fret9); %convert from RGB  to CIE 1976 L*ab
...
[cluster_idx, cluster_center] = kmeans(ab,nColors,'distance','sqEuclidean', 'Replicates',3);%k means clustering is used to create 4 colour clusters using the Eucilidean spaceMetric
...
vessel = fret9; % the image that has been filtered using Kirsch Operator
...
Z = imabsdiff(vc,mono2); % compare the images by subtracting the groundtruth from my result
